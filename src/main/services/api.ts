...
// URL, the session token as a bearer on every request,
...
/** The deployed Worker (`worker/`), where a build with no override goes. */
export const DEFAULT_API_URL = 'https://newline-api.newline-api.workers.dev'

/**
 * `MAIN_VITE_API_URL` overrides the default — `http://127.0.0.1:8787` for a
 * Worker running under `npm run dev`. electron-vite inlines
 * `import.meta.env.MAIN_VITE_*` at build time; `process.env` carries the same
 * name in a dev shell and under vitest. Read on every call so a test can
 * change its mind.
 */
