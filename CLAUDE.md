...
- Test: `npm test` (vitest, two projects: `renderer` in jsdom, `main` in node); `npm run test:watch`
...
│   ├── services/         notes, images/media, sync-status; each with __tests__/
...
└── renderer/src/         React app (alias `@/*`)
    ...
    ├── lib/              firebase, sync-service, R2 upload client
